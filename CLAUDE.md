...
- The recruiting engine and fit-scoring module (`src/lib/fit/`) are a walled-off module: no import of anything Bridge-specific (board/governance, donor/fundraising) may ever appear under `src/lib/fit/` or its future Doc AI counterpart. This is what makes the eventual standalone recruiting app a lift-and-shift instead of a rewrite.
