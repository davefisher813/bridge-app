...
- **The preview** (`scripts/preview/build_app_preview.ts`) is not a mockup. It renders every page in `src/testing/pages.ts`, the same list the render law executes, with the page code itself on the fixture and the app's own compiled stylesheet, into one tappable file. Links between screens work because every reachable screen is in the file. Six hand-written generators and a 2,700 line prototype used to do this job as a second implementation of the screens; they drifted from the app three times in one sitting and were retired in the clean slate (2026-09-19). Do not bring a hand-written mockup of a shipped screen back: if a screen needs to be seen, it is rendered.
- **The test bench** (`scripts/build_testbench.py` + `testbench_entry.ts`) bundles the SHIPPED `src/lib/fit/` and `src/lib/docai/` with esbuild and runs them in the page, so Dave can change an input and watch the real code respond, and the check list is the repo's own laws executing in his browser. This only works because both modules are walled off from Next/Supabase/DOM - that is the practical payoff of the rule, so do not weaken it for convenience.
