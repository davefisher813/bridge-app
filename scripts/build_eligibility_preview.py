...
def tile(label, value, sub=""):
    s = f'<div class="mt-0.5 text-[10.5px] text-muted">{sub}</div>' if sub else ""
    return ('<div class="rounded-[12px] bg-paper p-3.5">'
            f'<div class="text-[10.5px] font-bold uppercase tracking-[0.03em] text-muted">{label}</div>'
...
def verdict(status, role, line):
    ...
    return ('<div class="mb-4 rounded-[16px] bg-paper p-4">'
            ...
            f'<div class="text-[14px] font-bold leading-tight text-ink">{line}</div></div>')
...
<main class="px-4 pt-2 pb-6">
  <div class="mb-4"><a href="#" class="text-[13px] font-bold text-muted">&larr; Xavier I.</a></div>
  ...
  <div class="flex flex-col gap-2">
    {rail("committed", f'''<div class="flex items-center justify-between gap-3">
      <div class="min-w-0"><div class="text-[13px] font-bold text-ink">English</div>
      <div class="text-[11.5px] text-muted">4 credits &middot; best grades used</div></div>
      <span class="text-[13px] font-extrabold tabular-nums text-ink">3.00</span></div>''')}
    {rail("committed", f'''<div class="flex items-center justify-between gap-3">
      <div class="min-w-0"><div class="text-[13px] font-bold text-ink">Math</div>
      <div class="text-[11.5px] text-muted">3 credits</div></div>
      <span class="text-[13px] font-extrabold tabular-nums text-ink">2.33</span></div>''')}
    {rail("committed", f'''<div class="flex items-center justify-between gap-3">
      <div class="min-w-0"><div class="text-[13px] font-bold text-ink">Science</div>
      <div class="text-[11.5px] text-muted">2 credits</div></div>
      <span class="text-[13px] font-extrabold tabular-nums text-ink">2.50</span></div>''')}
    {rail("committed", f'''<div class="flex items-center justify-between gap-3">
      <div class="min-w-0"><div class="text-[13px] font-bold text-ink">Social science</div>
      <div class="text-[11.5px] text-muted">2 credits</div></div>
      <span class="text-[13px] font-extrabold tabular-nums text-ink">3.00</span></div>''')}
    {rail("committed", f'''<div class="flex items-center justify-between gap-3">
      <div class="min-w-0"><div class="text-[13px] font-bold text-ink">Other academic</div>
      <div class="text-[11.5px] text-muted">5 credits</div></div>
      <span class="text-[13px] font-extrabold tabular-nums text-ink">2.80</span></div>''')}
  </div>

  <div class="mb-2 mt-5">{header("Timing", None, "time")}</div>
  {rail("committed", f'''<div class="flex items-center gap-3">{badge("time", CLOCK)}
    <div class="min-w-0 flex-1"><div class="text-[13px] font-bold text-ink">10/7 rule met</div>
    <div class="text-[11.5px] text-muted">11 core credits, 8 in English, math or science, before senior year</div></div></div>''')}
  <div class="mt-2">{rail("committed", f'''<div class="flex items-center gap-3">{badge("time", CLOCK)}
    <div class="min-w-0 flex-1"><div class="text-[13px] font-bold text-ink">Five-year clock starts at enrollment</div>
    <div class="text-[11.5px] text-muted">Turns 19 after September 1, so nothing starts early</div></div></div>''')}

  <p class="mt-5 text-[11px] leading-relaxed text-muted">Projection until all 16 credits are final. Confirm with the NCAA Eligibility Center before anyone signs anything.</p>
</main>
""")
...
<main class="px-4 pt-2 pb-6">
  <div class="mb-4"><a href="#" class="text-[13px] font-bold text-muted">&larr; Brandon J.</a></div>
  <h1 class="mb-1 text-[20px] font-extrabold text-ink">NCAA eligibility</h1>
  <p class="mb-5 text-[12.5px] text-muted">Division I standard.</p>
  ...
  <div class="flex flex-col gap-2">
    {rail("offer", '''<div class="flex items-center justify-between gap-3">
      <div class="min-w-0"><div class="text-[13px] font-bold text-ink">Math</div>
      <div class="text-[11.5px] text-muted">3 credits, all D or C</div></div>
      <span class="text-[13px] font-extrabold tabular-nums text-ink">1.33</span></div>''')}
    {rail("offer", '''<div class="flex items-center justify-between gap-3">
      <div class="min-w-0"><div class="text-[13px] font-bold text-ink">Science</div>
      <div class="text-[11.5px] text-muted">2 credits</div></div>
      <span class="text-[13px] font-extrabold tabular-nums text-ink">2.00</span></div>''')}
  </div>

  <div class="mb-2 mt-5">{header("Not counted", "6 courses", "target")}</div>
  {rail("target", '''<div class="text-[12.5px] leading-tight text-ink">Phys. Ed. 9, 10 and 11, Art 1, Music App., Graphic Design.
    <div class="mt-1 text-[11.5px] text-muted">Not on the school's NCAA-approved list, so they raise the transcript average and not this one.</div></div>''')}
...
<main class="px-4 pt-2 pb-6">
  <div class="mb-4"><a href="#" class="text-[13px] font-bold text-muted">&larr; Kengri D.</a></div>
  <h1 class="mb-1 text-[20px] font-extrabold text-ink">NCAA eligibility</h1>
  <p class="mb-5 text-[12.5px] text-muted">Division I standard.</p>
  ...
  <div class="mb-4">{rail("offer", f'''<div class="flex items-start gap-3">{badge("time", WARN)}
    <div class="min-w-0 flex-1">
      <div class="text-[13px] font-bold text-ink">2.0 of 5 years gone before he enrolls</div>
      <div class="mt-1 text-[12px] leading-tight text-muted">He turns 19 on 15 March, before the September 1 cutoff, so the clock starts 1 August that year whether or not he has enrolled anywhere. A post-grad year spends eligibility while nobody is playing.</div>
    </div></div>''')}</div>
  ...
  <div class="flex flex-col gap-2">
    {rail("time", f'''<div class="flex items-center gap-3">{badge("time", CLOCK)}
      <div class="min-w-0 flex-1"><div class="text-[13px] font-bold text-ink">Runs continuously</div>
      <div class="text-[11.5px] text-muted">Does not pause for a redshirt, a transfer, a gap year or time away</div></div></div>''')}
    {rail("time", f'''<div class="flex items-center gap-3">{badge("time", CLOCK)}
      <div class="min-w-0 flex-1"><div class="text-[13px] font-bold text-ink">Expires Aug 2032</div>
      <div class="text-[11.5px] text-muted">Five years from the start, D1 and D2 only</div></div></div>''')}
  </div>
  ...
  <p class="mt-5 text-[11px] leading-relaxed text-muted">Dates are approximate to within a term: the clock starts at the start of the academic year, which each school sets itself.</p>
</main>
""")
...
<main class="px-4 pt-2 pb-6">
  ...
  <div class="flex flex-col gap-2">
    {rail("target", f'''<div class="flex items-start gap-3">{badge("place", BOOK)}
      <div class="min-w-0 flex-1">
        <div class="text-[13px] font-bold text-ink">James Monroe's grading scale is not on file</div>
        <div class="mt-1 text-[12px] leading-tight text-muted">His grades are numbers, not letters. The NCAA converts them using the school's own published table, so an 85 is not automatically a B. Ask the counselor for the conversion table, or upload a transcript that prints it.</div>
      </div></div>''')}
    {rail("target", f'''<div class="flex items-start gap-3">{badge("place", BOOK)}
      <div class="min-w-0 flex-1">
        <div class="text-[13px] font-bold text-ink">No NCAA-approved course list checked</div>
        <div class="mt-1 text-[12px] leading-tight text-muted">18 courses read off the transcript, none yet matched against what this school has approved with the Eligibility Center.</div>
      </div></div>''')}
  </div>
...
<main class="px-4 pt-2 pb-6">
  <div class="mb-4"><a href="#" class="text-[13px] font-bold text-muted">&larr; Angel V.</a></div>
  <h1 class="mb-1 text-[20px] font-extrabold text-ink">NCAA eligibility</h1>
  <p class="mb-5 text-[12.5px] text-muted">Target school is Division III.</p>

  {verdict("Does not apply", "low", "Division III sets its own academic standards on each campus.")}

  <div class="flex flex-col gap-2">
    {rail("contact", '''<div class="text-[12.5px] leading-tight text-ink">There is no NCAA core-course GPA for Division III.
      <div class="mt-1 text-[11.5px] text-muted">No 16-course requirement, no qualifier status, no minimum. Admission and eligibility are the school's decision, so the number to talk about is whatever that school's admissions office uses.</div></div>''')}
    {rail("contact", '''<div class="text-[12.5px] leading-tight text-ink">He still needs an NCAA ID.
      <div class="mt-1 text-[11.5px] text-muted">Division III athletes register with the Eligibility Center for the ID. Certification is only required for international athletes.</div></div>''')}
  </div>
  ...
  {rail("target", '''<div class="flex items-center justify-between gap-3">
    <div class="min-w-0"><div class="text-[13px] font-bold text-ink">Transcript GPA</div>
    <div class="text-[11.5px] text-muted">The school's own number, unconverted</div></div>
    <span class="text-[13px] font-extrabold tabular-nums text-ink">3.40</span></div>''')}
</main>
""")
